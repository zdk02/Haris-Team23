...
def _pipeline(agents=None, policy=None):
    ...
    # The MAPPING form, matching what the shipped pipeline uses. This used to be
    # `["recipient", "subject"]` — the identity-copy shape that left SubjectBindingAgent
    # inert in production, because the graph's `subject` never became `data_subject`. The
    # list form is still supported API and is covered in tests/test_trust_boundary.py; it
    # does not belong in the test that stands in for the real wiring.
    w_summ = haris.wrap(summarizer, "summarizer", "emailer",
                        data_type="summary", message_key="summary",
                        state_metadata_keys={"recipient": "recipient",
...
def test_metadata_carries_data_type_and_recipient():
    ...
    # `data_subject`, not `subject`: the agents read the former, and asserting the latter is
    # what let the wiring bug look correct for a whole phase.
    assert lineage[1].metadata.get("data_subject") == "patient-B"
